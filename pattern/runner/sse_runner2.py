...
class SSERunner(object):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            repeated_GU_pattern = np.repeat(np.expand_dims(GU_pattern, 0), planning_size, axis=0)
            assert planning_ABS_patterns.shape == (planning_size, self.K, self.K), f'{planning_ABS_patterns.shape}'
            assert repeated_GU_pattern.shape == (planning_size, self.K, self.K), f'{repeated_GU_pattern.shape}'

            # get incidces of top k transitions
            top_k_GU_patterns, top_k_ABS_patterns, top_k_pred_CGU_patterns = self.plan(repeated_GU_pattern, planning_ABS_patterns) # each has a shape of (top_k, K, K)
    ...
    def emulator_pretrain(self):
        ...
        for _episode in range(episodes):
            ...
            replay.add(data)
            print(f'[emulator pretrain: collecting] collected memory {replay.size}/{replay.max_size}')
    ...
    def policy_sample_ABS_patterns(self, GU_pattern, planning_size):
        ...
        pred_ABS_pattern = _t2n(self.policy.model.predict(GU_pattern, device=self.device)) # (1, 1, K, K)
    ...
    def plan(self, GU_patterns, planning_ABS_patterns):
        ...
        P_GUs = torch.FloatTensor(GU_patterns).unsqueeze(1).to(self.device)
        P_ABSs = torch.FloatTensor(planning_ABS_patterns).unsqueeze(1).to(self.device)

        # feed in the emulator to get predicted CGU_patterns
        if planning_size < self.planning_batch_size: # predict directly
            y_hats = _t2n(self.emulator.model.predict(P_GUs, P_ABSs, device=self.device)).squeeze() # (planning_size, K, K)
            ...
            for _P_GUs, _P_ABSs in zip(P_GUs_chunks, P_ABSs_chunks):
                _y_hats = _t2n(self.emulator.model.predict(_P_GUs, _P_ABSs, device=self.device)).squeeze()
                y_hats_chunks.append(_y_hats)
            y_hats = np.concatenate(y_hats_chunks)
            assert y_hats.shape == (planning_size, self.K, self.K), f"{(planning_size, self.K, self.K)}"
        pred_CGU_patterns = y_hats.reshape(planning_size, self.K, self.K)
        pred_CRs = np.sum(pred_CGU_patterns.reshape(planning_size, -1), axis=-1)
    ...
    def eval(self, curr_episode):
        ...
        with temp_seed(self.seed + 100000):
            ...
            for episode in range(self.eval_episodes):
                ...
                repeated_GU_pattern = np.repeat(np.expand_dims(GU_pattern, 0), planning_size, axis=0) # (planning_size, K, K)
                assert planning_ABS_patterns.shape == (planning_size, self.K, self.K), f"{planning_ABS_patterns.shape}"
                assert repeated_GU_pattern.shape == (planning_size, self.K, self.K)

                _, top_k_ABS_patterns, _ = self.plan(repeated_GU_pattern, planning_ABS_patterns)
                top_k_CGU_patterns = np.zeros_like(top_k_ABS_patterns)
